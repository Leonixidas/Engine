...
imp::Level1::Level1(const std::shared_ptr<MiniginGame>& game)
	:Scene(std::move("Level1"), game)
{
}

void imp::Level1::Initialize()
{
	LoadLevel();
...
void imp::Level1::Render() const
{
}

void imp::Level1::LoadLevel()
{
	std::ifstream input{};

	input.open(std::move("../Data/Level1.txt"));
...
void imp::Level1::CreatePlayer(size_t colum, size_t row)
{
...
void imp::Level1::CreatePooka(size_t , size_t )
{
}

void imp::Level1::CreateFygar(size_t , size_t )
{
}

void imp::Level1::CreateGrid(size_t colum, size_t row, size_t w, size_t h, const std::vector<size_t>& freeSpaces)
{
	for (size_t i = 0, size = colum * row; i < size; ++i)
	{
		if (std::find(freeSpaces.begin(), freeSpaces.end(), i) == freeSpaces.end())
		{
			...
			tile->GetTransform().SetPosition(float(w) * (i % colum), 640.f - 576.f + float(h) * (i / colum), 0.f);
